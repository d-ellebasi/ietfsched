 ...
 //           AnalyticsUtils.getInstance(getActivity()).trackPageView("/Sandbox");
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (cursor != null) {
            ...
            countSessions = cursor.getInt(TracksAdapter.TracksQuery.SESSIONS_COUNT);
//            Log.d(TAG, "CountSessions" + countSessions);
        } else {
...
//					Log.d(TAG, "Single Session / skip the sessions list, uri " + trackSessionsURI);
			...
			intent.setData(sessionId == null ? trackSessionsURI : ScheduleContract.Sessions.buildSessionUri(sessionId));
            }
        } else if (NEXT_TYPE_VENDORS.equals(mNextType)) {
/*            if (cursor == null) {
